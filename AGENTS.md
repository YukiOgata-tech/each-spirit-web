...
- `app/` はルート、レイアウト、metadata、loading UI、`sitemap.ts`、`robots.ts`、route handler を管理します。ルートは **大カテゴリ × section** 構造（`app/{food,health,beauty,travel,leisure}/[section]/**`）。`[section]` ページは既知 section を専用実装に委譲し、それ以外は汎用ページにフォールバックします。記事詳細は section 配下ではなく、すべて `/articles/{category}/{slug}` に集約します（`lib/section-map.ts` が canonical path を一元決定、section 定義は `es.content_sections`）。
