...
- `app/` はルート、レイアウト、metadata、loading UI、`sitemap.ts`、`robots.ts`、route handler を管理します。ルートは **大カテゴリ × section** 構造（`app/{food,health,beauty,travel,entertainment,leisure}/[section]/**`。`entertainment` は専用ルート実装あり）。`[section]` ページは既知 section を専用実装に委譲し、それ以外は汎用ページにフォールバックします。記事詳細は section 配下ではなく、すべて `/articles/{category}/{slug}` に集約します（`lib/section-map.ts` が canonical path を一元決定、section 定義は `es.content_sections`）。
- `components/` は再利用 UI です。`layout/`・`cards/`・`seo/`・`ui/` のほか、汎用 section ページ（`generic/`）、専用デザインの section 実装（`legacy-pages/{major}/{section}/**`、`app` から委譲呼び出し）、記事/ランキングの汎用ルート（`articles/`）、投稿 UI（`admin/`・`account/`）などに分かれます。
- `content/` はカテゴリ・地域などの設定と、旧 seed 用の型付きデータを置きます。記事本文・店舗/商品・ランキングの公開データは Supabase `es` スキーマを正とし、配信時は `lib/content.ts` が Supabase から読み取ります（protein 含め全カテゴリ移行済み。`content/**` の設定系ファイルのみ直接配信）。`npm run db:seed` は旧ローカル content で DB を上書きできるため通常実行では停止し、初期投入・復旧時のみ `ALLOW_LEGACY_CONTENT_SEED=1` を明示します。コンテンツページは ISR（`app/layout.tsx` の `revalidate = 2592000`、30日）で、即時反映は `app/api/revalidate/route.ts` の on-demand revalidate を使います。root layout 配下で `cookies()`/`headers()` を使うと ISR が無効化されるので注意。
- `lib/` は型、ルート定義（`routes.ts`）、大カテゴリ × section の変換と canonical path 生成（`section-map.ts`）、SEO 補助、コンテンツ取得関数（`content.ts`）、投稿 UI スキーマ（`admin-*.ts`）、運勢機能（`fortune*.ts`）、共通ユーティリティを置きます。Supabase クライアントは `lib/supabase/`（ブラウザ用 `client.ts` / SSR 用 `server.ts`）と `lib/supabase-server.ts`（service-role、seed 専用）に分かれます。
