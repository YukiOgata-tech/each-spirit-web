...
export async function generateMetadata({ params }: PageProps) {
  ...
  const article = await getArticleBySection("travel", "services", slug);
  if (!article) return {};
  return pageMetadata({ title: article.title, description: article.description, path: routes.sectionArticle("travel", "services", slug), image: article.coverImageUrl ?? site.icon });
}

export default async function TravelServiceArticlePage({ params }: PageProps) {
  const { slug } = await params;
