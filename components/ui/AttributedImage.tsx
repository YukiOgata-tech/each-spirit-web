...
export function AttributedImage({
  ...
  if (variant === "hover") {
    // hover variant はクリック可能カード（<Link>）内で使う前提のため、
    // クレジットは入れ子 <a> を避け装飾用 <span>（pointer-events-none）で表示する。
    return (
      <div className={`group/attr relative ${fill ? "h-full w-full" : ""} ${wrapperClassName ?? ""}`}>
        {img}
        <span className="pointer-events-none absolute inset-x-0 bottom-0 flex items-end justify-end bg-gradient-to-t from-black/60 to-transparent px-2.5 py-2 opacity-0 transition-opacity duration-200 group-hover/attr:opacity-100">
