...
import { Car, CloudSun, MapPin } from "lucide-react";
...
export function LeisureSpotCard({ region, spot }: { region: string; spot: LeisureSpot }) {
  ...
  return (
    <Card className="h-full overflow-hidden border-[var(--border)] transition hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative aspect-[16/9] overflow-hidden">
        <Image src={visual.imageUrl} alt={visual.imageAlt} fill className="object-cover transition duration-500 hover:scale-105" sizes="(min-width: 768px) 33vw, 100vw" />
      ...
      <CardHeader>
        ...
        <CardTitle>
          <Link href={routes.leisureSpot(region, spot.slug)} className="hover:text-[var(--primary)]">{spot.name}</Link>
        </CardTitle>
      </CardHeader>
