  ...
  return (
    <div className="xl:hidden">
      <motion.article
        ...
        className="relative overflow-hidden rounded-[28px] border border-white/15 bg-[#100d28] px-4 py-6 text-white shadow-2xl shadow-black/40 sm:px-7 sm:py-8"
      >
        ...
        <div className="relative">
          <header className="text-center">
            <p className="text-[11px] font-semibold text-white/45">{result.date}</p>
            <h1 className="mt-2 text-2xl font-bold tracking-normal text-white flex items-center justify-center gap-2">
          ...
          <section className="mt-4 rounded-3xl border border-white/12 bg-white/[0.07] px-3 py-5 text-center shadow-xl shadow-black/20 backdrop-blur">
          ...
          <section className="mt-5 rounded-3xl border border-white/10 bg-white/[0.06] px-5 py-6 text-center backdrop-blur">
            <SectionTitle>今日のメッセージ</SectionTitle>
            <p className="mt-4 text-sm leading-7 text-white/76">
              {strongest ? `${strongest.label}が今日の追い風です。${strongest.text}` : result.overall.text}
            </p>
            {focus ? (
              <p className="mt-3 border-t border-white/10 pt-3 text-xs leading-6 text-white/48">
...
function CategoryCard({ category }: { category: FortuneScore }) {
  ...
  return (
    <div
      className="relative min-w-0 overflow-hidden rounded-2xl border px-3 py-3 shadow-lg shadow-black/20"
      style={{
