...
export async function generateStaticParams() {
  const pairs = await Promise.all(getBeautyRegions().map(async (region) => ({ salons: await getBeautySalons(region.slug) })));
...
export default async function BeautySectionSalonPage({ params }: PageProps) {
  ...
  const pairs = await Promise.all(getBeautyRegions().map(async (region) => ({ region: region.slug, salons: await getBeautySalons(region.slug) })));
